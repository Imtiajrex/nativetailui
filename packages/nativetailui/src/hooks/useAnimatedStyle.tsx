...
export function useGroupedAnimatedStyle<T extends ViewStyle | TextStyle>({
	...
	const tw = useTw();
	const { animatableClasses, groupHoverClasses, groupActiveClasses } =
		separateClasses(className, isText);
	...
	const state = useMotiPressable(
		...
		[groupActiveStyles, groupHoverStyles, animatableStyles]
	);
...
export const useAnimatedStyle = <T extends ViewStyle | TextStyle>({
	...
	} = useMemo(() => separateClasses(className, isText), [className, isText]);
	...
	return {
		...
		exit: tw`${outClasses}`,
		style: [tw`${nonAnimatableClasses}`, style],
		textClasses,
		state,
	};
};
